...
UTFT_Button::UTFT_Button(void) {
   _gfx = 0;
}

void UTFT_Button::initButton(UTFT *gfx,
					  int16_t x, int16_t y, 
					  uint8_t w, uint8_t h, 
					  ...
					  char *label)
{
  ...
  _labelx = _x - (strlen(_label)/2) * _gfx->getFontXsize();
  if((strlen(_label) % 2) != 0 )
    _labelx -= _gfx->getFontXsize()/2;
  _labely = _y - _gfx->getFontYsize()/2;
}

 

 void UTFT_Button::drawButton(boolean inverted) {
   ...
   if (! inverted) {
    ...
    _gfx->setColor(fill);
   _gfx->fillRoundRect(_x - (_w/2), _y - (_h/2), _x + (_w/2), _y + (_h/2) );
   _gfx->setColor(outline);
   _gfx->drawRoundRect(_x - (_w/2), _y - (_h/2), _x + (_w/2), _y + (_h/2));
   
   
   _gfx->setFont(BigFont);
   _gfx->setColor(text);
   _gfx->setBackColor(fill);
   _gfx->print(_label, _labelx, _labely);
 }

boolean UTFT_Button::contains(int16_t x, int16_t y) {
 ...
 void UTFT_Button::press(boolean p) {
   laststate = currstate;
   currstate = p;
 }
 
 boolean UTFT_Button::isPressed() { return currstate; }
 boolean UTFT_Button::justPressed() { return (currstate && !laststate); }
 boolean UTFT_Button::justReleased() { return (!currstate && laststate); }


// code for the GFX button NumBox element

UTFT_NumBox::UTFT_NumBox(void) {
   _gfx = 0;
}

void UTFT_NumBox::initNumBox(UTFT *gfx, int16_t x1, int16_t y1, int16_t x2, int16_t y2,
		      ...
		      char *label)
{
 ...
 void UTFT_NumBox::drawNumBox(int16_t num) {

   _gfx->setFont(SmallFont);
   _gfx->setColor(_textcolor);
   _gfx->setBackColor(0x0);
   _gfx->print(_label, _labelx, _labely);
   
    _gfx->setColor(_fill);
   _gfx->fillRoundRect(_x1, _y1, _x2, _y2);
   _gfx->setColor(_outline);
   _gfx->drawRoundRect(_x1, _y1, _x2, _y2);

   _gfx->setFont(BigFont);
    _gfx->setBackColor(_fill);
    _gfx->setColor(0x0);
    _gfx->printNumI(num, _x1 + 10, _y1 + (_y2 - _y1)/3);
